...
struct LtrCalibData {
  size_t processedTFs{};               ///< number of processed TFs
...
class CalibLaserTracks
{
 public:
  ...
  /// at least 50 tracks per side per TF
  bool hasEnoughData(size_t numTFs = 1) const { return mZmatchPairsA.size() > 50 * numTFs && mZmatchPairsC.size() > 50 * numTFs; }
