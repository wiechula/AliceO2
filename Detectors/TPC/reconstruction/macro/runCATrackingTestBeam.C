...
int runCATrackingTestBeam(TString filename = "", TString outputFile = "", TString options = "", bool mergeChain = false,
                  int nmaxEvent = -1, int startEvent = 0)
{
  ...
  c.SetBranchAddress("runNumber",          &eventHeader.run);
  c.SetBranchAddress("cherenkovValue",     &eventHeader.cherenkovValue);
  c.SetBranchAddress("beamMomentum",       &eventHeader.beamMomentum);
  c.SetBranchAddress("powerSupply",        &eventHeader.powerSupply);
  c.SetBranchAddress("HVSettings",         &eventHeader.HVSettings);
  c.SetBranchAddress("trigger",            &eventHeader.trigger);
  c.SetBranchAddress("dataType",           &eventHeader.dataType);
  c.SetBranchAddress("driftFieldStrength", &eventHeader.driftFieldStrength);
  c.SetBranchAddress("runType",            &eventHeader.runType);
  ...
  tout.Branch("header", &eventHeader, "run/I:cherenkovValue/F:beamMomentum/I:powerSupply:HVSettings:trigger:dataType:driftFieldStrength:runType");
