...
void addData(AliceO2::TPC::GBTFrameContainer& container, std::string& infile, int frames, std::string rorcFlavor, int& done) {
  done = 0;
  mtx.lock();
...
int main(int argc, char *argv[])
{
  ...
  if (printGbtFrames) {
    for (std::vector<AliceO2::TPC::GBTFrameContainer*>::iterator it = container.begin(); it != container.end(); ++it) {
      ...
      for (std::vector<AliceO2::TPC::GBTFrame>::iterator itt = (*it)->begin(); itt != (*it)->end(); ++itt) {
