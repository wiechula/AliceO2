...
public:
  ...
  /// return the global pad number of PadROCPos (ROC, row, pad)
  ...
  const int getNumberOfRowsROC(int roc)             const { return (roc<getNumberOfIROCs()) ? mNumberOfPadRowsIROC : mNumberOfPadRowsOROC; }
  ...
  const int getNumberOfRowsPartition(int partition) const { return mMapPartitionInfo[partition % getNumberOfPartitions()].getNumberOfPadRows(); }
