...
void Detector::SetSpecialPhysicsCuts()
{
  ...
  // call base class function to load default cuts, overwite below if necessary
  o2::base::Detector::SetSpecialPhysicsCuts();

  const auto& detectorParam = DetectorParam::Instance();

  // ===| Overwrite for Krypton simulation |====================================
  //
  // Krypton simulation requires more relaxed cuts and the old Geant3 kloss model
  // due to soft electrons in the spectrum
  // This will not work with Geant4
  if (detectorParam.SimType == DetectorParam::SimulationType::Krypton) {
    LOGP(info, "[TPC]: Using simulation cut overwrites for Krypton simulation");
    const float cutTofmax = 1e10;
    const float cutDeltaEle = 1e-5;
    const float cutDefault = 1e-6;
    const int klossModel = 5;
...
Bool_t Detector::ProcessHits(FairVolume* vol)
{
  ...
  } else if (detectorParam.SimType == DetectorParam::SimulationType::Krypton) {
    LOGP(info, "[TPC]: Using energy loss model for Krypton simulation");
