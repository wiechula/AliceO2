...
//  const DigitPos digiPadPos = mapper.findDigitPosFromGlobalPosition(GlobalPosition3D(pos.X(), pos.Y(), pos.Z()));

//  TClonesArray& clrefSec = *mPointSector[digiPadPos.getPadSecPos().getSector().getSector()];
//  Int_t sizeSec = clrefSec.GetEntriesFast();
//  return new(clrefSec[sizeSec]) Point(trackID, detID, pos, mom,
//         time, length, eLoss);
